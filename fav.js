...
const favoriteBtnPager = document.querySelector("#favorite-btn-pager")
const favoriteCardsUl = document.querySelector("#favorite_ul")
...
function renderFav(){
    ...
    const fullCards = JSON.parse(localStorage.getItem("array"))
    ...
    favoriteCardsUl.innerHTML = html
    
    favoriteCardsUl.addEventListener("click", (e) => {
        const target = e.target
        if(target.closest(".card-1_btn-favorite")){

            const card = e.target.closest(".card-collage_el")
            const attribute = card.getAttribute("data-page")

            const changedFav = favoriteImages.filter((w, id) => id !== 0)

            localStorage.setItem("array", JSON.stringify(changedFav))
